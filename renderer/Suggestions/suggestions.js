(function () {
  ...
  function highlight(text, query, cls) {
    ...
    if (i === -1) { push(text, true); return frag; }
  ...
  function render(payload) {
    ...
    items.forEach((item, idx) => {
      ...
      icon.className = 'fav'; icon.alt = '';
      let fallback = SVG_GLOBE;
      if (search)                          fallback = SVG_SEARCH;
      else if (item.type === 'history')    fallback = SVG_HIST;
      else if (item.type === 'bookmark')   fallback = SVG_BKMK;
      else if (item.type === 'switch-tab') fallback = SVG_TAB;
      ...
      const addSecondary = (text, opts) => {
        ...
        if (opts && opts.url) sec.appendChild(highlight(text, query, 'url'));
        else sec.textContent = text;
      ...
      if (search) {
        ...
        if (item.type === 'action') addSecondary('Search with ' + (ENGINE_NAME[engine] || 'Google'));
        ...
        main.appendChild(primary);
        if (item.type === 'switch-tab') addSecondary('Switch to Tab');
        else if (item.url) addSecondary(item.url, { url: true });
        el.title = item.url || '';
      }
      ...
      el.addEventListener('mousedown', (e) => {
        e.preventDefault();
        try { window.overlaySuggestions.pointerDown && window.overlaySuggestions.pointerDown(); } catch {}
