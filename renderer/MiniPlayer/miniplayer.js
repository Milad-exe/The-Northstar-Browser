...
api.onState((s) => {
    ...
    if (s.art) { art.src = s.art; art.style.display = ''; }
    ...
    if (document.activeElement !== vol) vol.value = Math.round((s.vol ?? 1) * 100);
});
...
document.getElementById('x-track').addEventListener('click', (e) => {
    ...
    if (r.width > 0) api.action('seek', (e.clientX - r.left) / r.width);
});
