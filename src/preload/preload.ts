...
(function passwordManager() {
    ...
    function autofill() {
        ipcRenderer.invoke('passwords-get-for-origin').then((creds) => {
            ...
            const pw = document.querySelector('input[type="password"]:not([disabled])') as HTMLInputElement | null;
            if (!pw) return;
            const cred = creds[0];
    ...
    document.addEventListener('click', (e) => {
        try {
            const btn = (e.target as any)?.closest?.('button,[type="submit"],[role="button"]');
