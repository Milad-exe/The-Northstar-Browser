...
(function () {
    ...
    const prefs = Object.assign(
        { fontIndex: 2, face: 'sans', width: 'medium', theme: 'light' },
        loadPrefs()
    );
    ...
    document.getElementById('rd-font-dec').addEventListener('click', () => {
        prefs.fontIndex = Math.max(0, prefs.fontIndex - 1); applyPrefs(); savePrefs();
    });
    document.getElementById('rd-font-inc').addEventListener('click', () => {
        prefs.fontIndex = Math.min(FONT_SIZES.length - 1, prefs.fontIndex + 1); applyPrefs(); savePrefs();
    });
    document.querySelectorAll('#rd-typeface .rd-seg').forEach(b =>
        b.addEventListener('click', () => { prefs.face = b.dataset.face; applyPrefs(); savePrefs(); }));
    document.querySelectorAll('#rd-width .rd-seg').forEach(b =>
        b.addEventListener('click', () => { prefs.width = b.dataset.width; applyPrefs(); savePrefs(); }));
    ...
    window.northstarReader.getArticle().then((article) => {
        ...
        if (!article.byline) document.getElementById('rd-byline').remove();
