...
class DownloadManager {
    items: Map<number, any>;    // id → plain record (safe to send over IPC)
    handles: Map<number, any>;  // id → live Electron DownloadItem
    listeners: Set<any>;        // subscribers to download-list changes
    nextId: number;
    ...
    _emit(record) {
        for (const fn of this.listeners) {
            try { fn(record ? { ...record } : null); } catch {}
        }
    }
    ...
    hasActive() {
        for (const r of this.items.values()) {
            if (r.state === 'progressing') return true;
        }
        return false;
    }

    cancel(id)  { try { this.handles.get(id)?.cancel(); } catch {} }
    pause(id)   { try { this.handles.get(id)?.pause(); }  catch {} }
    resume(id)  { try { this.handles.get(id)?.resume(); } catch {} }

    openFile(id) {
        const r = this.items.get(id);
        if (r?.state === 'completed') { try { shell.openPath(r.savePath); } catch {} }
    }

    showInFolder(id) {
        const r = this.items.get(id);
        if (r) { try { shell.showItemInFolder(r.savePath); } catch {} }
    }

    remove(id) {
        const r = this.items.get(id);
        if (!r || r.state === 'progressing') return;
    ...
    clearFinished() {
        ...
        for (const [id, r] of this.items) {
            if (r.state !== 'progressing') { this.items.delete(id); changed = true; }
        }
...
function uniquePath(dir, filename) {
    ...
    for (let n = 1; n < 1000; n++) {
        try { if (!fs.existsSync(candidate)) break; } catch { break; }
