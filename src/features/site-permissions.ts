...
class SitePermissions extends EventEmitter {
    MANAGED: Array<{ name: string; label: string }>;  // permissions this store manages
    file: string | null;        // encrypted store path (userData)
    data: any;                  // { perms: { origin → { perm → 'allow'|'deny' } }, sitesOff }
    loaded: boolean;
    _anyOff: boolean;           // fast path: any per-site protection toggles off?
...
export default new SitePermissions();