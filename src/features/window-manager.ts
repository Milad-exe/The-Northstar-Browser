...
import contextMenu from './window-context-menu';
class WindowManager {
    windows: Map<number, any>;      // windowId → { id, window, tabs, shortcuts, menu, … overlays }
    nextWindowId: number;
    lastFocusedWindowId: number | null;  // most recently focused window (persistence primary)
    restored: boolean;              // saved tab state already restored into a window?
    cachedHistory: History | null;
    cachedBookmarks: Bookmarks | null;
    cachedPersistence: Persistence | null;
    ...
    _persistWindowBounds(window, options: any = {}) {
        try {
    ...
    createWindow(width = 800, height = 600, options: any = {}) {
        ...
        window.webContents.on("context-menu", async (_event, params: any) => {
