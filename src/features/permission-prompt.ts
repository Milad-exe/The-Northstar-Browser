...
import { systemPreferences, webContents, desktopCapturer } from 'electron';
import sitePermissions from './site-permissions';
import * as permissionUI from './permission-ui';
/**
...
function labelFor(names) {
    if (names.includes('camera') && names.includes('microphone')) return 'use your camera and microphone';
...
function iconForNames(names) {
    ...
    if (names.includes('microphone')) return 'microphone';
    if (names.includes('location')) return 'location';
    if (names.includes('notifications')) return 'notifications';
    return 'generic';
}
const ICON_FOR_PERM = {
...
function clearTempOrigin(rec, origin) {
    for (const k of [...rec.temp.keys()]) if (k.split('|')[1] === origin) rec.temp.delete(k);
}
...
sitePermissions.on('change', (origin) => {
    for (const rec of sessions.values()) if (rec.persist) clearTempOrigin(rec, origin);
});
...
function setOverride(sess, origin, name, value) {
    ...
    if (!rec || !origin) return;
    if (value === 'allow' || value === 'block') rec.override.set(`${origin}|${name}`, value);
    else rec.override.delete(`${origin}|${name}`);
...
function listStates(sess, origin) {
    return sitePermissions.MANAGED.map(p =>
        ({ name: p.name, label: p.label, state: effectiveState(sess, origin, p.name) }));
}
...
function hasTempAllow(origin, name) {
    for (const rec of sessions.values()) {
        for (const [k, allowed] of rec.temp) {
            if (!allowed) continue;
            const parts = k.split('|');
            if (parts[1] === origin && parts[2].split(',').includes(name)) return true;
        }
    }
    return false;
}
...
function attach(sess, { persist = true } = {}) {
    ...
    const trackWc = (wc) => {
        if (!wc || wc._permPromptTracked) return;
        wc._permPromptTracked = true;
        const clear = () => { for (const k of [...temp.keys()]) if (k.startsWith(wc.id + '|')) temp.delete(k); };
    ...
    sess.setPermissionRequestHandler(async (wc, permission, callback, details) => {
        try {
            ...
            if (names && names.length === 0) return callback(true); // media playback / speaker pick — not a capture gate
            if (names && names.length) {
                ...
                if (states.some(s => s === 'block')) return callback(false);
                if (states.every(s => s === 'allow')) return grant(true);
                trackWc(wc);
                const tempKey = `${wc?.id ?? 0}|${origin}|${names.join()}`;
                if (temp.has(tempKey)) return grant(temp.get(tempKey));
                const { allowed, remember, dismissed } = await prompt(wc, origin, labelFor(names),
                    { checkbox: persist, iconType: iconForNames(names) });
                if (remember && persist) {
                    for (const n of names) sitePermissions.set(origin, n, allowed ? 'allow' : 'block');
                } else if (!dismissed) {
            ...
            if (PROMPT_ONCE[permission]) {
                ...
                if (temp.has(tempKey)) return callback(temp.get(tempKey));
                const { allowed, dismissed } = await prompt(wc, origin, PROMPT_ONCE[permission],
                    { iconType: ICON_FOR_PERM[permission] || 'generic' });
                if (!dismissed) temp.set(tempKey, allowed);
                return callback(allowed);
            }
    ...
    sess.setDisplayMediaRequestHandler(async (req, callback) => {
        ...
        try {
            ...
            if (!allowed) return respond(null);
            const sources = await desktopCapturer.getSources({ types: ['screen'] });
            if (!sources.length) return respond(null);
            respond({ video: sources[0] });
        } catch { respond(null); }
    });
}

export { attach, detach, setOverride, effectiveState, listStates, hasTempAllow, liveUse };