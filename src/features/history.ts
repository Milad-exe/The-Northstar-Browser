...
class History {
    file: string | null;                       // JSON store path (userData)
    cache: any[] | null;                       // the single source of truth once loaded
    initialized: boolean;
    _writeTimer: ReturnType<typeof setTimeout> | null;  // debounced flush
    _quitHooked: boolean;
